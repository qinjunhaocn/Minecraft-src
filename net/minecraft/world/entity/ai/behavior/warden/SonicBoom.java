...
extends Behavior<Warden> {
    ...
    protected void tick(ServerLevel $$0, Warden $$12, long $$22) {
        ...
        $$12.getBrain().getMemory(MemoryModuleType.ATTACK_TARGET).filter($$12::canTargetEntity).filter($$1 -> $$12.closerThan((Entity)$$1, 15.0, 20.0)).ifPresent($$2 -> {
            Vec3 $$3 = $$12.position().add($$12.getAttachments().get(EntityAttachment.WARDEN_CHEST, 0, $$12.getYRot()));
            ...
            if ($$2.hurtServer($$0, $$0.damageSources().sonicBoom($$12), 10.0f)) {
                double $$9 = 0.5 * (1.0 - $$2.getAttributeValue(Attributes.KNOCKBACK_RESISTANCE));
                double $$10 = 2.5 * (1.0 - $$2.getAttributeValue(Attributes.KNOCKBACK_RESISTANCE));
                $$2.push($$5.x() * $$10, $$5.y() * $$9, $$5.z() * $$10);
            }
        });
    }
